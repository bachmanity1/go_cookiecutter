...
docker: ; $(info $(M) building docker image… ) @ ## Build for docker image 
	$Q $(SUDO) docker build --cache-from se/$(PACKAGE):latest --build-arg BUILD_PORT=$(DOCKERPORT) -t se/$(PACKAGE) .
...
ifeq (,$(wildcard $(CURDIR)/pkg.list))
	...
	$Q cd $(BASE) && GO111MODULE=on $(GO) list -f '{{.Dir}}' ./... 2>&1 | grep -v "^go: " | grep -v "^$(PACKAGE)/vendor/" | grep -v nocompile | grep -v logs | grep -v testset > pkg.list
endif
...
vendor: vendor-check go.mod pkg.list ; $(info $(M) retrieving dependencies…)
